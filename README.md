...
| LinkedinShareButton          | -                                                                                           | **`title`** (string): Title of the shared page<br/>**`summary`** (string): Description of the shared page<br/>**`source`** (string): Source of the content (e.g. your website or application name). These fields are best-effort only and may be ignored by LinkedIn.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
- `TwitterShareButton` keeps its historical name for backwards compatibility even though X is the current brand.
- X, LinkedIn, Facebook Messenger, and other networks can change which fields they honor without a major release from this library.
- `WhatsappShareButton` generates an `api.whatsapp.com` share URL.
- `ThreadsShareButton` still accepts legacy `hashtags`, `related`, and `via` props in v5 for compatibility, but they are ignored and will be removed in v6.
...
- Facebook Messenger: mobile support depends on Meta's current app and dialog behavior. `appId` is required, and `redirectUri` may still be needed for your integration flow.
- LinkedIn and X: share text, title, summary, or source fields may be ignored, rewritten, or populated from the shared page metadata instead.
